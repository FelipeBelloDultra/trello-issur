...
- **Outbox pattern** — `CreateAccountHandler` writes the account and its domain events (`outbox_events`) atomically through a single **shared** `UnitOfWork` port (`src/shared/database/application/repositories/unit-of-work.ts`, not one per module — see `apps/api/README.md`'s "Outbox pattern" section for why), then `OutboxRelay` (`src/infra/queue/outbox-relay.ts`) polls and publishes with a deterministic idempotency key (the outbox row's own id). Other publish-after-write handlers haven't been migrated yet — for those, a crash between the repository write and `QueuePublisherGateway.publish()` still silently drops the event; port them one at a time, don't assume atomicity until they do.
...
- **Cross-repository atomicity** goes through the shared `UnitOfWork` port (`src/shared/database/application/repositories/unit-of-work.ts`) — general-purpose, not tied to events/queue specifically, even though the outbox pattern is its only consumer today. Any repository can join by being built against `DrizzleExecutor` (`src/infra/db/transaction.ts`) instead of `DatabaseClient` directly. Migrate a repository to it **on demand**, only when a handler actually needs it inside a transaction — not preemptively across every repository, and not to force atomicity across two different aggregates/modules just because the mechanism allows it (one transaction should still stay scoped to one aggregate, or an aggregate plus its own technical side effects like outbox — see `apps/api/README.md`'s "Outbox pattern" section for the full reasoning).
- A repository built against `DrizzleExecutor` must be registered **without** `Lifecycle.Singleton` (see `apps/api/README.md`'s "Outbox pattern" → "DI registration gotcha") — tsyringe child containers share a singleton's cached instance by reference with the parent container, so a transaction-bound instance can silently leak into normal (pooled) request handling otherwise. Repositories still built against `DatabaseClient` directly are unaffected.
