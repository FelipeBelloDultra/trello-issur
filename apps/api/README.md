...
| Testing | Vitest |
| Package manager | pnpm ≥ 10 (workspace root) |

---

## Running locally

This app is part of the `trello-issur` pnpm workspace — run `pnpm install` from
the **repo root**; every other command uses `pnpm --filter api run <script>`
(or `cd apps/api && pnpm run <script>` directly):
...
pnpm --filter api run dev:queue   # queue consumer process (separate)
```
