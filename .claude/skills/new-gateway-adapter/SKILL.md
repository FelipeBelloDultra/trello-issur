...
A gateway is a port interface owned by `application/` (module-local, e.g. `PasswordHasherGateway`) or `apps/api/src/shared/<concern>/application/gateways/` (cross-module, e.g. `StorageGateway`), implemented by one or more concrete adapters in `infra/`. Application and domain code depend only on the interface.
...
1. Read the port interface first — e.g. `apps/api/src/shared/storage/application/gateways/storage.gateway.ts` — and implement every method. Look at the sibling adapters (`apps/api/src/infra/storage/adapters/{local,s3}/`) for the expected shape and error handling.
2. Put the new adapter at `apps/api/src/infra/<concern>/adapters/<vendor>/<vendor>-<concern>.gateway.ts`, `@injectable()`, implementing the port (and any lifecycle interface the port family uses, e.g. `StorageLifecycle.initialize()` for storage drivers that need bootstrapping like bucket creation).
3. Read config from `apps/api/src/config/env.ts` (`env.<X>`) — validate required env vars in the constructor and throw a clear `Error` if misconfigured (see `S3StorageGateway`'s constructor check), rather than failing obscurely later.
4. **Driver selection** happens in the concern's `container.ts` via an env flag, not via multiple DI registrations — e.g. storage:
   ...
   Add your new driver as another branch/value here, and extend the env schema's driver enum in `apps/api/src/config/env.ts`.
...
1. **Decide scope**: module-local (only one module needs it) → `apps/api/src/modules/<module>/application/gateways/<name>.gateway.ts`. Cross-module → `apps/api/src/shared/<concern>/application/gateways/<name>.gateway.ts` (create the `<concern>/application/gateways/` folder if new).
2. **Interface**: name methods by capability, not by vendor (`hash`/`verify`, not `argon2Hash`) — the port must stay implementation-agnostic:
   ...
   ```
3. **Adapter**: `apps/api/src/infra/<concern>/adapters/<vendor>/` (or `apps/api/src/modules/<module>/infra/<vendor>/` for a module-local one, e.g. `apps/api/src/modules/account/infra/argon2/`), `@injectable()`, implements the interface.
...
5. **Register**: create or extend a `container.ts` for the concern and call it from the owning module's `setup<Module>Module()` (module-local) or from `apps/api/src/infra/container/index.ts` (shared/cross-module) — follow the existing `setup*Container()` call order there.
